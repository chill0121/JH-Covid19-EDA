...
First we will mutate a dataframe to show deaths per day for the whole dataset, aka reality.
```{r Model - Reality}
...
- You can really see how the vaccine reduced the mortality rate, though it took awhile for it to take hold (maybe vaccine hold-outs and issues distributing, especially winter 2023.
- There is a high amount of seasonality frequency to this data.
- The seasonality will be difficult to model because the vaccine came out so quickly. We really only saw a portion of the seasonality (less than 1 year) occur and thus the data our model to train on is sparse in this sense.
...
fit <- auto.arima(data_ts[, 2], seasonal = FALSE)
forecast_no_vaccine <- forecast(fit, 365)
...
checkresiduals(forecast_no_vaccine)
```
